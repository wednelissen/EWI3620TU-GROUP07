...
public class StateHighScores implements GLEventListener, KeyListener,
		MouseListener {
	...
	private Window nameWindow = new Window(nameWindowCoords, screenWidth,
			screenHeight);
	private Window scoreWindow = new Window(scoreWindowCoords, screenWidth,
			screenHeight);
	private Window levelWindow = new Window(levelWindowCoords, screenWidth,
			screenHeight);

	/**
	 * Loads the High Score state on the given Canvas. Switches to the default
	 * cursor and adds the High Score Menu as KeyListener etc.
	 * 
	 * @param canvas
	 *            : The Canvas on which the High Score Menu will be drawn
	 * @param mazerunner
	 *            : The game which will be resumed when button resume is clicked
	 */
