...
public class MapMenu extends Window {
	...
	/**
	 * maakt de map aan waarin op ieder blok kan worden geklikt om er
	 * bijvoorbeeld een vloer, muur of deur aan toe te voegen. standaard is dit
	 * scherm nog niet gevuld met blokken. deze worden toegevoegd met behulp van
	 * de functie 'setTotoalBuildingBlocks'.
	 * 
	 * @param sizes
	...
	/**
	 * zorgt dat de opgegeven blokken in de breedte en in de hoogte worden
	 * geinitaliseerd en creert vervolgens deze objecten met behulp van de
	 * functie 'createTotoalBuildingBlocks, zodat ze getekent kunnen worden.
	 */
	...
	/**
	 * berekent voor ieder blok zijn positie in de map met bijbehorende breedte
	 * en hoogte. hierna worden deze bloken gecreert als objecten. ieder blok is
	 * dus een eigen object.
	 */
	public void createTotalBuildingBlocks() {
		...
		for (int j = 0; j < TotalBuildingBlockY; j++) {
			float BuildingBlock_LinksBovenY = originalSizes[1]
					+ (BuildingBlockSizeY * (j));
			for (int i = 0; i < TotalBuildingBlockX; i++) {
				float BuildingBlock_LinksBovenX = originalSizes[0]
						+ (BuildingBlockSizeX * (i));

				float[] BuildingBlockSizes = new float[] {
						BuildingBlock_LinksBovenX, BuildingBlock_LinksBovenY,
						BuildingBlockSizeX, BuildingBlockSizeY };

				// System.out.println(BuildingBlock_LinksBovenX+" , "+BuildingBlock_LinksBovenY+" , "+BuildingBlockSizeX+" , "
				// + BuildingBlockSizeY);
				BuildingBlocks[i][j] = new BuildingBlock(BuildingBlockSizes,
						screenWidth, screenHeight, i, j);
			}
		}
	}

	/**
	 * creert uit een eerder opgeslagen level de bijbehorende hoeveelheid
	 * blokken met de gesetten waardes zoals vloer,muur,deur.
	 * 
	 * @param floorPlan
	 *            String van 0,1 en 2 van de plattegrond van de Maze.
	 * @param LoadedWidth
	 *            integer van het aantal buildingblocks in de breedte.
	 * @param LoadedHeight
	 *            integer van het aantal buildingblocks in de hoogte.
	 */
	public void loadBuildingBlocks(String[] floorPlan, int LoadedWidth,
			int LoadedHeight) {
	...
	/**
	 * alle sleutels uit een opgelsagen level worden nagelopen om te
	 * controlleren welke deuren een key nodig hebben. deze deuren zullen worden
	 * geset dat deze een key nodig hebben.
	 * 
	 * @param loadedKeys
	 *            Arraylist van Key
	 */
	...
	/**
	 * 
	 * @return true indien het aantal ingegeven blokken in de breedte en hoogte
	 *         tussen de 0 en 101 liggen. geeft anders false.
	 */
	public boolean hasHeightAndWidth() {
		if (TotalBuildingBlockX > 0 && TotalBuildingBlockY > 0
				&& TotalBuildingBlockX < 101 & TotalBuildingBlockY < 101) {
			return true;
		} else if (TotalBuildingBlockX == 0 && TotalBuildingBlockY == 0) {
			return false;
		}
	...
	// geklikt kan worden zodat ieder element uit de maze items kunnen worden
	// toegekent
	/**
	 * tekent alle blokken met hun gesetten toestand. bijvoorbeeld een blok waar
	 * een deur is geset zal als deur worden getekent.
	 * 
	 * @param gl
	 */
...
//							loadedTexturesEditor.getTexture("doorEditor"));
	...
	/**
	 * wanneer het scherm reshaped, zullen ook alle blokken in de map gereshaped
	 * worden
	 * 
	 * @param screenWidthFrame
	 * @param screenHeightFrame
	...
	public void updateBlocks(int screenWidthFrame, int screenHeightFrame) {
		for (int j = 0; j < TotalBuildingBlockY; j++) {
			for (int i = 0; i < TotalBuildingBlockX; i++) {
				BuildingBlocks[i][j]
						.update(screenWidthFrame, screenHeightFrame);
			}
		}
	}
	 ...
	 * @param yclick
	 *            mouse click
	 * @return het BuildingBlock waar op is geklikt. als er niet op een
	 *         buildingblock is geklikt returnt deze functie null.
	 */
