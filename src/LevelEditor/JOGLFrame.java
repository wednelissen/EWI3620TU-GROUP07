...
/**
 * HEEL ALGEMEEN VERHAAL OVER DE OPZET VAN DE LEVEL EDITOR: (optioneel om te
 * lezen)
 * 
 * Om te beginnen is verder gewerkt aan de in assignment 1 gekregen opdracht.
 * Hier wordt een JOGL frame geopent waarin kleine rechthoeken worden getekent
 * waarop kan worden geklikt. deze kleine rechthoeken zijn allemaal al van te
 * voren gedefinieerd in het formaat van 800 bij 600. Het scherm zal automatisch
 * in deze verhouding worden geschaald, omdat alle rechthoeken vast liggen is er
 * een lange lijst met vooraf gedefinieerde coordinaten. Deze worden aangegeven
 * door middel van 'Coords' aan het einde van de naam. De klasse Window en
 * button zorgen voor het maken van de klikbare rechthoeken. De klasse 'mapMenu'
 * is de classe die zorgt dat er kleine vierkantjes(BuildingBlocks) in het
 * middden van het scherm worden getekent waar een eigenschap aan kan worden
 * toegekent zoals een muur of een deur. De klasse 'PlacedItemsMenu' is het
 * linker middelste grote rechthoek waarin geplaatste items verschijnen zoals
 * een bewaker of een sleutel. wanneer op een sleutel of bewaker wordt geklikt
 * zal deze in de map zichtbaar worden. De mogelijkheid bestaat om alle bewakers
 * en/of sleutels zichbaar te maken door in de rechthoek links onder aan te
 * klikken op show all. Het onderste rechthoek verschijnt alleen wanneer een
 * item extra opties heeft zoals bij sleutel het geval is indien hier een deur
 * aan moet worden gekopppelt. Een deur die geplaatst is zal blauw van kleur
 * zijn. Wanneer hier een sleutel aan is gekoppeld zal de deur groen kleuren.
 * 
 * @author mvanderreek
 * 
 */
public class JOGLFrame extends Frame implements GLEventListener, MouseListener,
	...
	// Classes met een Jframe die wel aangemaakt worden maar nog niet zichtbaar
	// zijn.
	...
	private float[] itemDoorCoords = new float[] { 5 + 2 * 48.75f, 5, 48.75f,
			100 };
	private float[] itemSpotCoords = new float[] { 5 + 3 * 48.75f, 5, 48.75f,
			100 };
	private float[] itemGuardianCoords = new float[] { 5, 105, 48.75f, 100 };
	private float[] itemKeyCoords = new float[] { 5 + 48.75f, 105, 48.75f, 100 };
	private float[] itemCameraCoords = new float[] { 5 + 2 * 48.75f, 105,
			48.75f, 100 };
	private float[] itemControlCenterCoords = new float[] { 5 + 3 * 48.75f,
			105, 48.75f, 100 };

	private float[] placedItemsCoords = new float[] { 5, 235, 195, 200 };

	private float[] placedItemsPropertiesCoords = new float[] { 5, 440, 195,
			155 };
	private float[] addGuardKeySpotCameraCoords = new float[] { 5, 440, 97.5f,
			77.5f };
	private float[] removeGuardKeySpotCameraCoords = new float[] { 102.5f, 440,
			97.5f, 77.5f };
	private float[] removeLastPointGuardOrSetDoorKeyCoords = new float[] { 5,
			517.5f, 97.5f, 77.5f };
	private float[] showAllGuardsKeysCoords = new float[] { 102.5f, 517.5f,
			97.5f, 77.5f };
	...
	private PlacedItemsMenu placedItems = new PlacedItemsMenu(
			placedItemsCoords, screenWidth, screenHeight);
	private Window placedItemsProperties = new Window(
			placedItemsPropertiesCoords, screenWidth, screenHeight);
	private Window Melding = new Window(MeldingCoords, screenWidth,
			screenHeight);
	private Window Melding2 = new Window(Melding2Coords, screenWidth,
			screenHeight);

	// define the buttons
	private Button setStart = new Button(setStartCoords, screenWidth,
			screenHeight);
	private Button setEnd = new Button(setEndCoords, screenWidth, screenHeight);
	private Button setSizes = new Button(setSizesCoords, screenWidth,
			screenHeight);
	...
	private Button itemFloor = new Button(itemFloorCoords, screenWidth,
			screenHeight);
	private Button itemWall = new Button(itemWallCoords, screenWidth,
			screenHeight);
	private Button itemDoor = new Button(itemDoorCoords, screenWidth,
			screenHeight);
	private Button itemSpot = new Button(itemSpotCoords, screenWidth,
			screenHeight);
	private Button itemGuardian = new Button(itemGuardianCoords, screenWidth,
			screenHeight);
	private Button itemKey = new Button(itemKeyCoords, screenWidth,
			screenHeight);
	private Button itemCamera = new Button(itemCameraCoords, screenWidth,
			screenHeight);
	private Button itemControlCenter = new Button(itemControlCenterCoords,
			screenWidth, screenHeight);

	private Button addGuardKeySpotCamera = new Button(
			addGuardKeySpotCameraCoords, screenWidth, screenHeight);
	private Button removeGuardKeySpotCamera = new Button(
			removeGuardKeySpotCameraCoords, screenWidth, screenHeight);
	private Button removeLastPointGuardOrSetDoorKey = new Button(
			removeLastPointGuardOrSetDoorKeyCoords, screenWidth, screenHeight);
	private Button showAllGuardsKeys = new Button(showAllGuardsKeysCoords,
			screenWidth, screenHeight);
	...
	// Arraylists, de Key en Guard worden in placedItems opgeslagen, op deze
	// wijze kun je ze nog selecteren en veranderen.
	 ...
	 * becomes visible. However there is no moving ´camera´, so the view and projection can 
	...
	/**
	 * In deze methode worden alle teken functies aangeroepen om de layout te
	 * tekenen. Afhankelijk van de ingedrukte button zal er meer of minder
	 * schermen zichtbaar zijn in de layout. Zo verschijnt er links onderaan een
	 * extra menu met opties wanneer er op Guard is gedrukt.
	 * 
	 * @param gl
	 */
	private void drawWindows(GL gl) {
		// als de breedte en lengte zijn ingegeven mogen de buildingBlocks
		// worden getekent in de map.
		...
		if (mapCreated) {
			...
			if (StartEnd.hasStart()) {
				...
				map.getBuildingBlockByPosition(a).drawBlock(gl,
						LoadTexturesEditor.getTexture("editorStartPos"));
			}
			...
			if (StartEnd.hasEnd()) {
				...
				map.getBuildingBlockByPosition(a).drawBlock(gl,
						LoadTexturesEditor.getTexture("editorEndPos"));
			}

			// alle guards zullen worden getekent met blauwe blokjes.
			// indien je 1 specifieke guard hebt geselecteerd word deze met rode
			// blokjes getekent
			if (AllGuardsOnOff) {
				for (Guardian g : placedItems.getAllGuards()) {
					for (int i = 0; i < g.routeSize(); i++) {
						Point a = g.getRoute(i);
						map.getBuildingBlockByPosition(a).drawGuardianPath(
								gl,
								LoadTexturesEditor
										.getTexture("editorGuardStepsBlue"));
					}
				}
			}
			...
			if (AllKeysOnOff) {
				for (Key k : placedItems.getAllKeys()) {
					...
					map.getBuildingBlockByPosition(a).drawKey(gl,
							LoadTexturesEditor.getTexture("editorKeyInMaze"));
				}
			}

			// er worden rode voetstappen getekend in de blokjes waar de guard
			// loopt. Dit geldt alleen voor deze ene geselecteerde guard.
			if (guard.routeSize() > 0 && Mode == ClickOptions.guardian) {
				for (int i = 0; i < guard.routeSize(); i++) {
					...
					map.getBuildingBlockByPosition(a).drawGuardianPath(
							gl,
							LoadTexturesEditor
									.getTexture("editorGuardStepsRed"));
				}

			}

			// sleutel die is geselecteerd wordt getekent
			if (key.hasPosition()
					&& (Mode == ClickOptions.key || Mode == ClickOptions.setKeyDoor)) {
				Point a = key.getKey();
				map.getBuildingBlockByPosition(a).drawKey(gl,
						LoadTexturesEditor.getTexture("editorKeyInMaze"));
			}
			...
			if (cameraList.getCameras().size() > 0) {
				for (Camera s : cameraList.getCameras()) {
					...
					map.getBuildingBlockByPosition(a)
							.drawCameras(
									gl,
									LoadTexturesEditor
											.getTexture("editorCameraInMaze"));
				}
			}

			// alle geplaatste control centers worden getekent
			if (controlCenterList.getControlCenters().size() > 0) {
				for (ControlCenterEditor s : controlCenterList
						.getControlCenters()) {
					Point a = s.getPosition();
					map.getBuildingBlockByPosition(a).drawControlCenter(
							gl,
							LoadTexturesEditor
									.getTexture("editorControlCentreInMaze"));
				}
			}
			...
			if (spotList.getSpots().size() > 0) {
				for (Spot s : spotList.getSpots()) {
					...
					map.getBuildingBlockByPosition(a).drawSpot(gl,
							LoadTexturesEditor.getTexture("editorSpot"));
				}
			}
			...
			if (!StartEnd.hasStart() && !StartEnd.hasEnd()) {
				gl.glColor3f(0, 0, 0);
				Melding2.renderString(gl,
						"can't save: Begin and End are not set.");
				gl.glColor3f(1, 1, 1);
			...
			map.draw(gl, LoadTexturesEditor.getTexture("editorMenuBackground"));
			gl.glColor3f(0, 0, 0);
			Melding.renderString(gl,
					"You need to set the maze sizes at 'Size',\n\tbefore you can add items.");
			gl.glColor3f(1, 1, 1);
		...
		itemControlCenter.draw(gl,
				LoadTexturesEditor.getTexture("controlCenterEditor"));

		// de items met speciale eigenschappen zoals Key en Guard worden hier
		// getekent.
		// dit zijn de items die al met een positie in de map zijn geplaatst.
		placedItems.draw(gl,
				LoadTexturesEditor.getTexture("editorMenuBackground"));
		...
		if (Mode == ClickOptions.guardian) {
			...
			addGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorAddGuardButton"));
			removeGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorRemoveButton"));
			removeLastPointGuardOrSetDoorKey.draw(gl,
					LoadTexturesEditor.getTexture("editorRemoveLastPoint"));
			showAllGuardsKeys.draw(gl,
					LoadTexturesEditor.getTexture("editorShowAllGuardsButton"));
		}
		...
		if (Mode == ClickOptions.key || Mode == ClickOptions.setKeyDoor) {
			...
			if (key.hasDoor()) {
				addGuardKeySpotCamera.draw(gl,
						LoadTexturesEditor.getTexture("editorAddKeyButton"));
			}
			removeGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorRemoveButton"));
			removeLastPointGuardOrSetDoorKey.draw(gl,
					LoadTexturesEditor.getTexture("editorSetDoorButton"));
			showAllGuardsKeys.draw(gl,
					LoadTexturesEditor.getTexture("editorShowAllKeysButton"));
		}
		...
		if (Mode == ClickOptions.setSpot || Mode == ClickOptions.removeSpot) {
			placedItemsProperties.draw(gl,
					LoadTexturesEditor.getTexture("editorMenuBackground"));
			addGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorSetSpotButton"));
			removeGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorRemoveButton"));
		}
		...
		if (Mode == ClickOptions.setCamera || Mode == ClickOptions.removeCamera) {
			placedItemsProperties.draw(gl,
					LoadTexturesEditor.getTexture("editorMenuBackground"));
			addGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorSetCameraButton"));
			removeGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorRemoveButton"));
		}

		if (Mode == ClickOptions.setControlCenter
				|| Mode == ClickOptions.removeControlCenter) {
			placedItemsProperties.draw(gl,
					LoadTexturesEditor.getTexture("editorMenuBackground"));
			addGuardKeySpotCamera.draw(gl, LoadTexturesEditor
					.getTexture("editorAddControlCenterButton"));
			removeGuardKeySpotCamera.draw(gl,
					LoadTexturesEditor.getTexture("editorRemoveButton"));
		}
	 ...
	 * 
	 * Er wordt gecontroleerd of op 1 van de windows of buttons is gedrukt. indien op deze button gedrukt zal er een actie 
	...
	public void mouseReleased(MouseEvent me) {
		...
		if (map.clickedOnIt(me.getX(), me.getY())) { // Map
		...
		} else if (itemControlCenter.clickedOnIt(me.getX(), me.getY())) { // Control
																			// Center
			System.out.println("control center");
			controlCenter = new ControlCenterEditor();
			Mode = ClickOptions.setControlCenter;
		} else if (placedItems.clickedOnIt(me.getX(), me.getY())) { // placed
																	// Items
																	// (kunnen
																	// Guard of
																	// Keys
		...
		} else if (placedItemsProperties.clickedOnIt(me.getX(), me.getY())) { // extra
																				// opties
																				// menu
		...
		} else if (setSizes.clickedOnIt(me.getX(), me.getY())) { // set nieuwe
																	// size van
																	// Maze
			sizes.appear();
			...
			if (SaveLevel != null) {
				...
				StoreMaze = new SaveInput(map, placedItems, StartEnd, spotList,
						cameraList, controlCenterList, SaveLevel);
			}
	...
	/**
	 * Wanneer het nieuwe level wordt ingeladen zal alles tot nu toe worden
	 * gereset en daarna worden alle objecten uit newlevel ingeladen, zodat er
	 * verder bewerkt kan worden
	 * 
	 * @param newlevel
	 *            Het level dat moet worden ingeladen
	 */
	private void initialiseObjects(LoadLevel newlevel) {
		this.resetObjects();
		map.loadBuildingBlocks(newlevel.getFloorPlan(), newlevel.getWidth(),
				newlevel.getHeight());
	...
	/**
	 * Wanneer op de map is geklikt zal worden gekeken welke Mode is geset en
	 * zal dit afgehandeld worden
	 * 
	 * @param me
	 */
	private void mapClickHandler(MouseEvent me) {
		if (map.BuildingBlocksExists()) {
			if (Mode == ClickOptions.wall) { // Wall
				...
				if (setWall) {
					removeKeyAndDoor(me); // als op deze plek een deur stond
											// word de deur met bijbehorende
											// sleutel verwijderd
					map.getClickedBuildingBlock(me.getX(), me.getY()).setWall();
				} else
					System.out
							.println("er staat een bewaker/begin punt/eind punt op deze plek");
			}
			...
			if (Mode == ClickOptions.door) { // Door
				// de door wordt geset, wanneer er geen bewaker/begin punt/eind
				// punt op deze plek staat.
			...
			if (Mode == ClickOptions.setSpot) { // place Spot
				...
				BuildingBlock temp = map.getClickedBuildingBlock(me.getX(),
						me.getY());
			...
			if (Mode == ClickOptions.removeControlCenter) { // Remove
															// ControlCenter
			...
			if (Mode == ClickOptions.guardian) { // Guard
				/*
				 * kijkt of het geklikte vlakje een floor bevat en voegt dit
				 * punt toe aan de route lijst van de bewaker. Dit punt moet wel
				 * horizontaal of verticaal grenzen aan het vorig aangeklikte
				 * punt waar de bewaker loopt. de bewaker mag niet schuin lopen
				 */
				BuildingBlock temp = map.getClickedBuildingBlock(me.getX(),
						me.getY());
				if (temp.getFloor()) {
					...
					System.out
							.println("dit is geen open plek, dus er kan geen bewaker worden geplaatst");
			}
			...
			if (Mode == ClickOptions.key) { // Key
				...
				if (temp.getFloor()) {
					...
					System.out
							.println("dit is geen open plek, dus er kan geen key worden geplaatst");
			}
			if (Mode == ClickOptions.setKeyDoor) { // Connect Key to Door
				...
				if (temp.getDoor() && !temp.getKeyRequired()) {
					...
					// er is wordt hij verwijderd en word de keyRequired uit
					// gezet.
					if (key.hasDoor()) {
						Point a = key.getDoor();
					...
					System.out
							.println("geen deur of deur heeft al een sleutel");
			}
			...
			if (Mode == ClickOptions.setStart) { // Set Start
				...
				if (temp.getFloor()) {
					...
					System.out
							.println("de start positie kan alleen op een open vlak");
			}
			...
			if (Mode == ClickOptions.setEnd) { // Set End
				...
				if (temp.getFloor()) {
					...
					System.out
							.println("de eind positie kan alleen op een open vlak");

			}
		} else
			System.out.println("Vul een lengte en breedte in.");
	}

	/**
	 * Wanneer dit BuildingBlock een deur is die een sleutel nodig heeft, zal de
	 * deur met de bijbehorende sleutel worden verwijderd. als er geen sleutel
	 * bij de deur hoort gebeurt er niets.
	 * 
	 * @param me
	 */
	...
	/**
	 * Kijkt of het mogelijk is om een muur of deur te plaatsen. Dit is niet
	 * mogelijk wanneer er een spot, camera, sleutel of een bewaker route of een
	 * begin of eindpunt staat. Indien het wel mogelijk is returnt de funtie
	 * true.
	 * 
	 * @param me
	 * @return True wanneer er een muur of deur kan worden geplaatst
	 */
	private boolean posibleToPlaceWallOrDoor(MouseEvent me) {
		boolean possible = true;

		// mag niet wanneer er een Guardian path loopt.
		Point a = map.getClickedBuildingBlock(me.getX(), me.getY())
				.getPosition();
		for (Guardian g : placedItems.getAllGuards()) {
		...
		if ((StartEnd.getStart().equals(a) && StartEnd.hasStart())
				|| (StartEnd.getEnd().equals(a) && StartEnd.hasEnd())) {
			possible = false;
		}
		return possible;
	}
	...
	/**
	 * Wanneer het extra menu zichtbaar is en er is op geklikt zal hier het
	 * bijbehoordende event worden afgehandeld. Voor een Guard geld: - het
	 * toevoegen van een guard - het verwijderen van de guard - een route punt 1
	 * stap terug nemen - alle geplaatste guards zichtbaar maken
	 * 
	 * voor Key: - het toevoegen van een key - het verwijderen van een key -
	 * deur aan een key koppelen - alle keys zichtbaar maken
	 * 
	 * voor Spot: - toevoegen van een Spot - verwijderen van een Spot
	 * 
	 * voor Camera: - toevoegen van een Camera - verwijderen van een Camera
	 * 
	 * voor Control Center: - toevoegen van een Control Center - verwijderen van
	 * een Control Center
	 * 
	 * @param me
	 */
	private void PlacedItemsPropertiesClickHandler(MouseEvent me) {
		if (Mode == ClickOptions.guardian) { // Guard
			if (addGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
				...
				Guardian copyGuard = new Guardian(itemCoords, screenWidth,
						screenHeight);
				copyGuard.setTotalRoute(guard.getCopyRoutes(),
						guard.geXprevious(), guard.geYprevious());
			...
			if (removeLastPointGuardOrSetDoorKey.clickedOnIt(me.getX(),
					me.getY())) {
			...
			if (removeGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
				if (key.hasDoor()) {
					Point a = key.getDoor();
					map.getBuildingBlockByPosition(a).removeKeyRequired();
					System.out.println(map.getBuildingBlockByPosition(a)
							.getKeyRequired());
				}
		...
		else if (Mode == ClickOptions.setSpot
				|| Mode == ClickOptions.removeSpot) { // Spot
			if (addGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
				Mode = ClickOptions.setSpot;
			}
			if (removeGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
		...
		else if (Mode == ClickOptions.setCamera
				|| Mode == ClickOptions.removeCamera) { // Camera
			if (addGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
				Mode = ClickOptions.setCamera;
			}
			if (removeGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
		...
		else if (Mode == ClickOptions.setControlCenter
				|| Mode == ClickOptions.removeControlCenter) { // Control Center
			if (addGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
				Mode = ClickOptions.setControlCenter;
			}
			if (removeGuardKeySpotCamera.clickedOnIt(me.getX(), me.getY())) {
	...
	/**
	 * Alleen wanneer een goede waarde is opgeslagen zal de if statement 1 maal
	 * worden uitgevoerd totdat opnieuw een nieuwe geldige waarde is ingevuld in
	 * het SizeWindows.
	 */
	...
	private void resetObjects() {
		...
		placedItems = new PlacedItemsMenu(placedItemsCoords, screenWidth,
				screenHeight);
