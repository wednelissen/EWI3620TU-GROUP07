 ...
 * TO DO:
 * 
 * PROBLEMEN: omdat key nog niet is geimplementeerd is er een probleem. je kan
 * hem wel toevoegen maar nog niet verwijderen.
 * 
 * 
 * 
 * KEYS: key moet nog helemaal worden geimplementeerd, dit lijkt erg sterk op
 * spots maar dan met een verwijzing naar een deur.
 * 
 * DOORS: er moeten deuren kunnen worden geplaatst in het level waar 1 of
 * meerdere sleutels aan is gekoppeld.
 * 
 * een Deur maak je in buildingBlock wanneer er later een sleutel is aangemaakt
 * kan deze aan een deur worden gekoppeld door te kijken of de building block
 * een door is. de key bevat dan de coordinaten van een door. wanneer er een
 * wall of floor wordt geset wordt gecontrolleerd of er een key nodig was voor
 * deze deur. zo ja dan wordt in de arraylist van keys, deze bevind zich in
 * PlacedItemsMenu gezocht naar de bij behorende deur en wordt de sleutel
 * verwijderd. bij het saven zal een deur worden gesaved als een 2. met behulp
 * van de arraylist keys kan je vinden voor welke deur een sleutel nodig was.
 * 
 * je kunt altijd nog implementeren dan je niet kunt saven voordat iedere deur
 * een keyrequired = true heeft.
 * 
 * 
 * LEVEL: een popup window voor de hoogte een popup window voor sava om een naam
 * in te geven en op te slaan een lijst met levels die je kunt spelen. het
 * inladen van een saved level om verder te bewerken in de editor. je kunt niet
 * saven voordat iedere deur een sleutel heeft en andersom.
 * 
 **/
...
public class JOGLFrame extends Frame implements GLEventListener, MouseListener,
	...
	private float[] removeGuardKeySpotCameraCoords = new float[] { 102.5f, 440,
			97.5f, 77.5f };
	private float[] removeLastPointGuardCoords = new float[] { 5, 517.5f,
			97.5f, 77.5f };
	private float[] showAllGuardsCoords = new float[] { 102.5f, 517.5f, 97.5f,
			77.5f };
	...
	private Button setHeight = new Button(setHeightCoords, screenWidth,
			screenHeight);
	private Button setWidth = new Button(setWidthCoords, screenWidth,
			screenHeight);
	...
	private Button removeLastPointGuard = new Button(
			removeLastPointGuardCoords, screenWidth, screenHeight);
	private Button showAllGuards = new Button(showAllGuardsCoords, screenWidth,
			screenHeight);

	private SaveInput StoreMaze;
	// private GuardianList allGuards = new GuardianList(placedItemsCoords,
	...
	private void drawWindows(GL gl) {
		...
		if (mapCreated) {
			...
			// er worden rode kruisjes getekend in de blokjes waar de guard
			// loopt.
	...
	public void mouseReleased(MouseEvent me) {
		...
		if (map.clickedOnIt(me.getX(), me.getY())) {
			...
			StoreMaze = new SaveInput(map, placedItems, StartEnd, spotList,
					cameraList); // de gegenereerde map wordt opgeslagen
			StoreMaze.floorPlanMaze(); // de map wordt vertaald naar enen en
										// nullen
			StoreMaze.GuardsPlan(); // de bewakers worden weggeschreven
			StoreMaze.StartAndEndPosition(); // begin en eindpositie worden naar
												// string vertaald.
			...
			if (StartEnd.hasStart() && StartEnd.hasEnd()) {
				StoreMaze.write("level1"); // de map wordt weggeschereven naar
											// een bestand.
			} else
	...
	private void mapClickHandler(MouseEvent me) {
		if (map.BuildingBlocksExists()) {
			if (Mode == ClickOptions.wall) {
				// de wall wordt geset, wanneer er geen bewaker/begin punt/eind
			...
			if (Mode == ClickOptions.guardian) {
				...
				if (temp.getFloor()) {
					...
					System.out
							.println("hier staat een muur er kan geen bewaker worden geplaatst");
			}
			...
			// System.out.println("wall = "+temp.getWall() + " floor = "+
			// temp.getFloor());
		} else
	...
	private void PlacedItemsPropertiesClickHandler(MouseEvent me) {
		...
		// zorgt dat de mode wordt geset om een spot te deleten of toe te voegen
		// wanneer op een plek op de map
		// wordt gedrukt.
	...
	public void keyReleased(KeyEvent event) {
		...
		case setHeight:
			if ((keycode > 95 && keycode < 106)
				...
				map.removeHeight();
				// nu moet ook de hele maze gereset worden!!!!!! dit moet nog
				// gemaakt worden
			}
			break;
