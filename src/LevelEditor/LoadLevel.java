...
public class LoadLevel {
	...
	// wordt niet gebruikt alleen ter initalisatie
	private float[] placedItemsCoords = new float[] { 5, 235, 195, 200 };

	/**
	 * Geeft de name van het uit te lezen bestand op en deze wordt met de
	 * scanner is losse strings verwerkt. de ID als eerste regel moet
	 * overeenkomen met de controllen String anders is het geen geldig level
	 * editor bestand.
	 * 
	 * @param name De source naam van het level dat moet worden ingeladen
	 */
	public LoadLevel(String name) {
		...
		try {
			...
			if (ID.equals("a5ir783n!f78gds3b?54sdfg>sdfg549fd#sh")) {
				valid = true;
				// breedte en hoogte worden opgevraagd en de maze wordt
				// ingelezen als enen en nullen.
				printWidth = sc.next();
				printHeight = sc.next();
				...
				sc.next();
				sc.next(); // laad het kopje met 'begin punt:'
				// System.out.println(sc.nextLine());
				StartPosition = sc.next();
				// System.out.println(StartPosition);
				sc.next();
				sc.next(); // laad het kopje met 'eind punt:'
				EndPosition = sc.next();

				// de spots worden ingelezen als cijfers die de punten
				// voorstellen waar een spot staat.
				...
				// de camera's worden ingelezen als cijfers die de punten
				// voorstellen waar een camera staat.
				...
				// de Control Centers worden ingelezen als cijfers die de punten
				// voorstellen waar een control center staat.
				...
				// de guards worden ingelezen als cijfers die de punten
				// voorstellen waar een bewaker loopt.
				...
				// de Keys worden ingelezen als cijfers. het eerste punt is de
				// sleutel het tweede punt de deur
	...
	/**
	 * 
	 * @return een string met de plattegrond van de maze. een vloer, muur of
	 *         deur, aangegeven met een 0, 1 of 2.
	 */
	...
	/**
	 * 
	 * @return een Arraylist van Punten waar de spotjes zich bevinden
	 */
	...
	/**
	 * 
	 * @return een Arraylist van Punten waar de camara's zich bevinden
	 */
	...
	/**
	 * 
	 * @return een Arraylist van Punten waar de control Centers zich bevinden
	 */
	...
	/**
	 * 
	 * @return een ArrayList van Guardians met hun routes.
	 */
	...
	/**
	 * 
	 * @return een Arraylist van Keys met hun positie en daarbijbehorende deur
	 */
	...
	/**
	 * 
	 * @return true wanneer het uitgelezen bestand voldoet aan de controlle ID,
	 *         geeft anders false.
	 */
